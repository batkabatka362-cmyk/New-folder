...
python -m memebot.backtest.swing_discover [--split 0.7]               # WL19 (Phase-3 / AGI core): autonomous strategy DISCOVERY + WALK-FORWARD validation. Searches mean-reversion + Bollinger param grids, judges each on a held-out TEST slice (train 70%/test 30%), PASS only if it beats buy-hold IN and OUT of sample + is profitable OOS. Automates the manual edge-hunt with the anti-overfit rigor it lacked. VERDICT: mean-rev holds OUT-OF-SAMPLE (te_gmean ~1.5, beats hold 14/15 OOS; best = w24 k0.18 = the swing default); Bollinger/vol-scaling is weaker. Seed of a propose->backtest->validate->paper-deploy loop.
